const licenses = [
  ...
  },
  {
    ...
    abbr: "GNU",
    url: "GNU General Public License (GPL)",
  },
  {
...
const findLicense = (license) => {
  for (lic of licenses) {
    if (lic.name === license) return lic;
  }
};
...
function renderLicenseBadge(license) {
  ...
  return lic
    ? `![license](https://img.shields.io/static/v1?label=license&message=${lic.abbr}&color=brightgreen)`
    : "";
}
...
function renderLicenseLink(license) {
  //Compare the selected license value from answers to that in the licenses object to select the abbr so I can set the badge
  let lic = findLicense(license);
  return lic ? lic.url : "";
}
